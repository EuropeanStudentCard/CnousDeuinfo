...
namespace DueInfo
{
    class Console_OpenSsl //: ISynchronizeInvoke
    {
        ...
        //const string app_name = "C:\\OpenSSL-Win64\\bin\\openssl.exe";
        ...
        const string verif_sign_args = "dgst -sha256 -verify {0} -signature {1} {2}";
        ...
        public string CreateArguments(ActionOpenSsl action, List<string> var)
        {
            ...
            {
                if (var.Count >= 3)
                    args = string.Format(verif_sign_args, var[0], var[1], var[2]);
                /*if (var.Count >= 4)
